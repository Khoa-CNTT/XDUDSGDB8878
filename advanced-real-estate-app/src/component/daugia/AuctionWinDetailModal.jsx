...
const AuctionWinDetailModal = (props) => {
    ...
    const handleChooseFileChange = (event) => {
        ...
        if (!file.type.startsWith("image/")) {
            setUploadStatus(`File ${name} phải là hình ảnh!`)
            return
        }
        if (file.size > 5 * 1024 * 1024) {
            setUploadStatus(`File ${name} không được vượt quá 5MB!`)
            return;
        }
        setFiles((prevFiles) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const data = await handleAPI(`/api/admin/auction-contracts`, formData, "POST", auth?.token);
            if(data?.error){
                ...
                <div className={`modal-content ${styles.modalContent}`}>
                    ...
                    <div className={`modal-body ${styles.modalBody}`}>
                        <div className={styles.contractSection}>
                            <h6 className={styles.sectionTitle}>{'Thủ tục pháp lý'.toUpperCase()}</h6>
                            <p>
                                ...
                                <span style={{
                                  ...
                                  Lưu ý: Vui lòng nhập vào hoặc chỉnh sửa lại chính xác và trung thực
                                  thông tin cá nhận của bạn để chúng tôi xác thực
                                  danh tính của bạn và tiến tới ký kết hợp đồng đấu giá với bạn
                                </span>
                            </p>
                            <div className={styles.inputGroup}>
                                ...
                                <span className={styles.errorText}>
                                    {elements?.forAuctionWinDetailModal?.errorMessage(errorMessages?.user_name)}
                                </span>
                            </div>
                            <div className={styles.inputGroup}>
                                ...
                                />
                                <span className={styles.errorText}>
                                    {elements?.forAuctionWinDetailModal?.errorMessage(errorMessages?.phone_number)}
                                </span>
                            </div>
                            <div className={styles.inputGroup}>
                                ...
                                />
                                <span className={styles.errorText}>
                                    {elements?.forAuctionWinDetailModal?.errorMessage(errorMessages?.birthday)}
                                </span>
                            </div>
                            <div className={styles.inputGroup}>
                                ...
                                />
                                <span className={styles.errorText}>
                                    {elements?.forAuctionWinDetailModal?.errorMessage(errorMessages?.address)}
                                </span>
                            </div>
                            <div className={styles.inputGroup}>
                                ...
                                />
                                <span className={styles.errorText}>
                                    {elements?.forAuctionWinDetailModal?.errorMessage(errorMessages?.note)}
                                </span>
                            </div>
                            <div className={styles.inputGroup}>
                        ...
                        <div className={styles.contractSection}>
                            ...
                            <div className={styles.infoItem}>
                                ...
                                <span>{props?.utils?.objectItem?.typeBuildingResponse?.type_name}</span>
                            </div>
                            <div className={styles.infoItem}>
                            ...
                            <div className={styles.infoItem}>
                                ...
                                <span>{props?.utils?.objectItem?.building?.number_of_basement}</span>
                            </div>
                            <div className={styles.infoItem}>
                                <i className={`fa fa-money ${styles.icon}`}></i>
                                <span className={styles.label}>Giá khởi điểm:</span>
                                <span>
                                    {appVariables.formatMoney(props?.utils?.objectItem?.typeBuildingResponse?.price)}
                                </span>
                            </div>
                            <div className={styles.infoItem}>
                            ...
                            <div className={styles.infoItem}>
                                <b style={{
                                   ...
                                   cursor: 'pointer'
                                    ...
                                    onClick={()=>{
                                        const modalElement = document.getElementById("auctionWinDetailModal");
                                        if (modalElement) {
